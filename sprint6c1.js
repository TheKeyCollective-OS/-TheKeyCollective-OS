...
// Option 1 collectible-figurine art direction: keep every autonomous surprise in the same cohesive visual family.
surprises.kiki.forEach(item=>{item[2]='assets/companion/kiki-option1.png';item[3]='Kiki in her polished collectible business-lady look'});
surprises.lulu.forEach(item=>{item[2]='assets/companion/lulu-option1.png';item[3]='Lulu in her plush mommy-mode cardigan'});
...
function triggerSurprise(){const cfg=companionConfig();if(!cfg.enabled||!cfg.surprises)return;const mode=activeMode(),list=surprises[mode],item=list[Math.floor(Math.random()*list.length)],host=document.querySelector('#kikiCompanion'),image=host.querySelector('img');clearTimeout(surpriseArtTimer);image.src=item[2];image.alt=item[3];host.classList.add('is-surprise-art');performMove(item[0],30000);say(item[1]);surpriseArtTimer=setTimeout(()=>{host.classList.remove('is-surprise-art');applySettings()},30000)}
window.addEventListener('lani:hidden-key',()=>{if(route==='lani'&&activeMode()==='lulu')triggerSurprise()});
...
function applySettings(){
  ...
  image.src=mode==='lulu'?'assets/companion/lulu-option1.png':'assets/companion/kiki-option1.png';image.alt=mode==='lulu'?'Lulu, your gentle dinosaur companion':'Kiki, your tiny dinosaur companion';
...
applySettings=()=>{applySettingsOriginal();const host=document.querySelector('#kikiCompanion'),cfg=companionConfig();if(!host)return;const mode=activeMode(),image=host.querySelector('img');image.src=mode==='lulu'?'assets/companion/lulu-option1.png':'assets/companion/kiki-option1.png';image.alt=mode==='lulu'?'Lulu, your gentle collectible dinosaur companion':'Kiki, your tiny collectible dinosaur companion'};
