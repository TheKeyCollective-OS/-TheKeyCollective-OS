...
const surprises={kiki:[['surprise-moonwalk','Girl, I’m checking the details. Somebody has to know what’s going on.'],['surprise-drag','Bestie, I found something interesting and obviously had to investigate.'],['surprise-present','Okay queen, I took notes. I have thoughts.'],['surprise-dance','Baby girl, I’m just minding your business professionally.']],lulu:[['surprise-present','Hey, Shug. Mommy mode is here—we brought the essentials.'],['surprise-sway','Love, the storybook and cozy blanket are ready whenever you are.'],['surprise-hug','Baby girl, you’re taking beautiful care of your little world.'],['surprise-sway','Sweetheart, let’s make room for one soft moment together.']]};
...
function triggerSurprise(){const cfg=companionConfig();if(!cfg.enabled||!cfg.surprises)return;const mode=activeMode(),list=surprises[mode],item=list[Math.floor(Math.random()*list.length)],host=document.querySelector('#kikiCompanion'),image=host.querySelector('img');clearTimeout(surpriseArtTimer);image.src=mode==='lulu'?'assets/companion/lulu-mommy.png':'assets/companion/kiki-busybody.png';image.alt=mode==='lulu'?'Lulu in nurturing mommy mode':'Kiki investigating with her clipboard';host.classList.add('is-surprise-art');performMove(item[0],4200);say(item[1]);surpriseArtTimer=setTimeout(()=>{host.classList.remove('is-surprise-art');applySettings()},4300)}
