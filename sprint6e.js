...
function premiumCardAtelier(){
  const state=store.get(),design=state.design||{},pack=design.pack||'executive',chosen=packs.find(item=>item[0]===pack)||packs[0],cardCollection=design.cardCollection||'pearl',tone=design.tone||'light',radius=['classic','square','circle'].includes(design.radius)?design.radius:'classic';
  ...
  <section class="card typography-experience-6c" data-pack="${pack}" data-radius="${radius}" data-card-collection="${cardCollection}" data-tone="${tone}">
...
function bindPremiumAtelier(){
  ...
  const sync=()=>{
    const design={...(store.get().design||{}),...draft()},pack=design.pack||'executive',tone=design.tone||'light',radius=design.radius||'classic',collection=design.cardCollection||'pearl';
    studio.dataset.pack=pack;studio.dataset.tone=tone;studio.dataset.radius=radius;studio.dataset.cardCollection=collection;
